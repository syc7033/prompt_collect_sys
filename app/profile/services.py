...
class ProfileService:
    ...
    async def get_user_statistics(db: Session, user_id: uuid.UUID) -> ProfileStatistics:
        ...
        # 获取用户提示词被使用的总次数
        total_usage = db.query(func.count(Usage.id)).join(Prompt, Usage.prompt_id == Prompt.id)\
            .filter(Prompt.creator_id == user_id).scalar() or 0
        
        # 获取用户提示词的平均评分
        avg_rating_query = db.query(func.avg(Prompt.average_rating)).filter(
            Prompt.creator_id == user_id,
            Prompt.rating_count > 0
        )
        logger.info(f"平均评分查询SQL: {str(avg_rating_query)}")
        
        avg_rating = avg_rating_query.scalar()
        logger.info(f"原始平均评分结果: {avg_rating}, 类型: {type(avg_rating)}")
        
        # 如果评分为None，设置为0.0
        if avg_rating is None:
            avg_rating = 0.0
            logger.info("评分为None，设置为0.0")
        else:
            logger.info(f"处理后的平均评分: {avg_rating}")
    ...
    @staticmethod
    async def count_user_prompts(db: Session, user_id: uuid.UUID) -> int:
        """获取用户创建的提示词总数"""
        return db.query(func.count(Prompt.id)).filter(Prompt.creator_id == user_id).scalar() or 0
    ...
    async def get_user_prompts(
        ...
        """获取用户创建的提示词列表"""
        try:
            logger.info(f"开始获取用户 {user_id} 的提示词列表: skip={skip}, limit={limit}, sort_by={sort_by}, sort_order={sort_order}")
            
            # 检查排序字段是否存在
            if not hasattr(Prompt, sort_by):
                logger.warning(f"排序字段 {sort_by} 不存在，使用默认字段 created_at")
                sort_by = "created_at"
            
            # 构建排序条件
            if sort_order.lower() == "asc":
                order_by = getattr(Prompt, sort_by)
                logger.debug(f"使用升序排序: {sort_by}")
            else:
                order_by = desc(getattr(Prompt, sort_by))
                logger.debug(f"使用降序排序: {sort_by}")
            
            # 查询用户创建的提示词
            query = db.query(Prompt).filter(Prompt.creator_id == user_id)
            logger.debug(f"查询SQL: {str(query)}")
            
            # 添加排序、分页并执行查询
            prompts = query.order_by(order_by).offset(skip).limit(limit).all()
            logger.info(f"查询到 {len(prompts)} 条提示词记录")
            
            # 转换为响应模型
            result = []
            for prompt in prompts:
                try:
                    item = UserPromptListItem.from_orm(prompt)
                    result.append(item)
                except Exception as e:
                    logger.error(f"转换提示词数据出错: {str(e)}, 提示词 ID: {prompt.id}")
            
            logger.info(f"成功转换 {len(result)} 条提示词数据")
            return result
        except Exception as e:
